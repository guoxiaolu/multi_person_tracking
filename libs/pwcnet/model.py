		...
		class Decoder(torch.nn.Module):
			...
			def forward(self, tensorFirst, tensorSecond, objectPrevious):
				...
				if objectPrevious is None:
					...
					tensorVolume = torch.nn.functional.leaky_relu(input=FunctionCorrelation(tensorFirst=tensorFirst, tensorSecond=tensorSecond), negative_slope=0.1, inplace=False)
					...
					tensorVolume = torch.nn.functional.leaky_relu(input=FunctionCorrelation(tensorFirst=tensorFirst, tensorSecond=Backward(tensorInput=tensorSecond, tensorFlow=tensorFlow * self.dblBackward)), negative_slope=0.1, inplace=False)
