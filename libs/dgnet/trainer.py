...
# Load model
#---------------------------
def load_network(network, name, trainer_pth_name):
    save_path = trainer_pth_name
    network.load_state_dict(torch.load(save_path))
    return network

def load_config(name, trainer_config_name):
    config_path = trainer_config_name
...
class DGNet_Trainer(nn.Module):
    def __init__(self, hyperparameters, trainer_pth_name, trainer_config_name, gpu_ids=[0]):
        ...
        if hyperparameters['teacher'] != "":
            ...
            for teacher_name in teacher_names:
                config_tmp = load_config(teacher_name, trainer_config_name)
                if 'stride' in config_tmp:
                    stride = config_tmp['stride'] 
                else:
                    stride = 2
                model_tmp = ft_net(ID_class, stride = stride)
                teacher_model_tmp = load_network(model_tmp, teacher_name, trainer_pth_name)
                ...
                if torch.cuda.is_available():
                    tmt = teacher_model_tmp.cuda().eval()
                else:
                    tmt = teacher_model_tmp.eval()
                teacher_model.append(tmt)
                teacher_count +=1
